...
public class ValidTriangleNumber {
    ...
    them as side lengths of a leetCode.twoDimentionArrays.triangle.

    Example 1:
    Input: nums = [2,2,3,4]
    Output: 3
    Explanation: Valid leetCode.backtracking.combinations are:
    ...
    public int triangleNumber(int[] nums) {
        //a valid leetCode.twoDimentionArrays.triangle needs at least 3 sides
        if (nums.length < 3) {
            return 0;
        }
        ...
        //start from the third value, the ith value will be value of one side of the leetCode.twoDimentionArrays.triangle.
        for (int i = 2; i < nums.length; i++) {
            //check the sub-arrays of length 2 before ith value to see which values can be used to make a valid leetCode.twoDimentionArrays.triangle
            int left = 0;
            int right = i - 1;
            while (left < right) {
                /*
                    A valid leetCode.twoDimentionArrays.triangle is when two sides summed are greater than once side, so here when left + right > ith
                    value, a valid leetCode.twoDimentionArrays.triangle was found
                 */
