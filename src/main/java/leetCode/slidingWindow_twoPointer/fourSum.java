...
public class fourSum {
    ...
    /*
        ...
        leetCode.slidingWindow_twoPointer.fourSum, equal target
     */
    private void threeSum(int curr_value, int left, int[] nums, int target, List<List<Integer>> set) {
        for (int i = left; i < nums.length - 2; i++) {
            ...
            if (i == left || nums[i] != nums[i - 1]) {
                //pass the first value from leetCode.slidingWindow_twoPointer.fourSum and current value in nums[i] and look for 2 other values to sum to target
