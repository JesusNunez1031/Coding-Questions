...
public class FindMinimumInRotatedSortedArrayII {
    ...
    public int findMin(int[] nums) {
        ...
        while (left < right) {
            ...
            // move the right pointer left if we are still on an ascending section of the array
            if (nums[mid] < nums[right]) {
                right = mid;
            }
            // move the left pointer right to the rotated part of the array, i.e. numbers of lesser value
            else if (nums[mid] > nums[right]) {
                left = mid + 1;
            }
            // if nums[mid] == nums[right] reduce the right pointer by one, this is done in the case of duplicates
            else {
                right--;
            }
        }
