...
public class SortingTheSentence {
    ...
    public String sortSentence(String s) {
        //split all the words in s into an array of words
        String[] words = s.split(" ");

        //array to hold the final sentence of words
        String[] sentence = new String[words.length];

        for (String word : words) {
            //get the index of where the word belongs in the sentence by extracting the digit from the word
            int index = Integer.parseInt(word.substring(word.length() - 1)) - 1;

            //set the word to its position in the sentence
            sentence[index] = word.substring(0, word.length() - 1);
        }

        StringBuilder sb = new StringBuilder();

        //append each word to the final string sentence adding a space in between each word minus the last
