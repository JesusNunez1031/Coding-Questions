...
public class richestCustomerWealth {
    ...
    Constraints:
        ...
        1 <= accounts[i][j] <= 100
     */
    //TC: O(n * m) where n is the number of customers and m is the length of wealth
    private int maximumWealth(int[][] accounts) {
        int richest = 0;

        for (int[] customer : accounts) {
            int customer_wealth = 0;
            //calculate the wealth of customer i
            for (int wealth : customer) {
                customer_wealth += wealth;
            }
            //if the wealth of the current customer is greater than the richest, the current customer becomes the new richest
