...
public class combinationSum {
    /*
    Given an array of distinct integers candidates and a target integer target, return a list of all unique leetCode.backtracking.combinations
    of candidates where the chosen numbers sum to target. You may return the leetCode.backtracking.combinations in any order.
    The same number may be chosen from candidates an unlimited number of times. Two leetCode.backtracking.combinations are unique if the
    frequency of at least one of the chosen numbers is different.

    It is guaranteed that the number of unique leetCode.backtracking.combinations that sum up to target is less than 150 leetCode.backtracking.combinations for the given input.
    ...
    These are the only two leetCode.backtracking.combinations.
    ...
    /*
        Method exhausts all possible choices for leetCode.backtracking.combinations to get to the target sum
        Ex: given [2, 3, 7] target = 7
            ...
            adding "duplicate" leetCode.backtracking.combinations, we also pass in the index we are at so as to avoid this, so when we are done
    ...
    private static void findCombinations(int[] candidates, List<List<Integer>> combinations, List<Integer> runningSum, int sum, int target, int start) {
        ...
        for (int i = start; i < candidates.length; i++) {
            ...
            if (sum < target) {
                ...
                //System.out.println(runningSum.toString());  //all possible leetCode.backtracking.combinations made
