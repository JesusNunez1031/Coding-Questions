...
public class partitionToKEqualSumSubsets {
    ...
    non-empty leetCode.backtracking.subsets whose sums are all equal.

    Example 1:
    Input: nums = [4, 3, 2, 3, 5, 2, 1], k = 4
    Output: True
    Explanation: It's possible to divide it into 4 leetCode.backtracking.subsets (5), (1, 4), (2,3), (2,3) with equal sums.

    Note:
        1 <= k <= len(nums) <= 16.
        0 < nums[i] < 10000.
     */
    //TC: ~O(2^n) since we make all possible leetCode.backtracking.subsets of nums, O(n) space used due to the recursive stack
    public boolean canPartitionKSubsets(int[] nums, int k) {
        //get the total sum of values, this will help determine of k leetCode.backtracking.subsets of equal sum are possible
        int total_sum = 0;
        for (int num : nums) {
            total_sum += num;
        }

        //if total_sum divided by k is not 0, we can't make k leetCode.backtracking.subsets since one or more leetCode.backtracking.subsets would have float values
        if (total_sum % k != 0) {
            return false;
        }

        //use backtracking to find all possible leetCode.backtracking.subsets and if k - 1 leetCode.backtracking.subsets are made, then k leetCode.backtracking.subsets of equal sum can be made
