public class SlowestKey {
    ...
    public char slowestKey(int[] releaseTimes, String keysPressed) {
        ...
        int longestDuration = releaseTimes[0]; // duration of the keypress with the longest time

        for (int i = 1; i < keysPressed.length(); i++) {
            char c = keysPressed.charAt(i);
            int duration = releaseTimes[i] - releaseTimes[i - 1]; // calculate duration of the current keypress

            /*
                update the longest keypress if the current key has a larger duration or if the duration matches the
                longest time and the current key is lexicographically larger than the previous key
             */
            if (duration > longestDuration || (duration == longestDuration && c > result)) {
                result = c;
