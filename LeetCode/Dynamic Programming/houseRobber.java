public class houseRobber {
    ...
    public int rob(int[] nums) {
        ...
        dp[0] = nums[0];
        //set the next value to the highest value, either its the current index, or the oe prior
        dp[1] = Math.max(dp[0], nums[1]);

        for (int i = 2; i < nums.length; i++) {
            //is the current home + the home before the last greater profit than if we would of robbed the home prior to this one?
